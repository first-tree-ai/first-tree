...
export function renderRuntimeOutputContract(): string {
  ...
  return [
    ...
    `- \`${bin} chat send <name> "<message>"\` — deliver a reply, or hand an agent its next step (informational only; a send states and never asks)`,
    `- \`${bin} chat ask <human> -F <body.md>\` — put any question to a human, blocking or not, including an optional offer or a one-line confirmation; the body is decision-self-sufficient: why the question exists + a recap of recent interactions + the single question and your recommendation`,
