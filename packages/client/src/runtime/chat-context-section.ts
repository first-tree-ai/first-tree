...
export function renderRuntimeOutputContract(): string {
  ...
  return [
    ...
    `- \`${bin} chat send <name> "<message>"\` — deliver a reply, or hand an agent its next step (informational only; a send states and never asks — a call you can settle yourself is settled and reported, not asked)`,
    `- \`${bin} chat ask <human> -F <body.md>\` — put a tracked decision to a human when your next step depends on it; the body is decision-self-sufficient: why the question exists + a recap of recent interactions + the single question and your recommendation`,
