  ...
  chat
    ...
    .description(
      "Send a message into the caller's current chat (FIRST_TREE_CHAT_ID). <name> is any participant — agent or " +
        ...
        "on from; a send never carries a question. A decision your next step depends on goes through " +
        "`chat ask`, which gives the answer a tracked slot; a smaller call you can settle from the task, the code, " +
        "or a reasonable default is settled and reported, not asked. Report progress with " +
        "`chat update --description`. A message must name a recipient " +
